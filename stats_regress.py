...
def sum_squared_error(true,pred):
    return np.sum(get_squared_error(true,pred))

def r2_score(true,pred):
    SSE = np.sum(get_squared_error(true,pred))
    shpe = len(np.array(true))
    SST = np.sum(get_squared_error(true,np.mean(true)*shpe))
    return 1.-(SSE/SST)
...
def assess_model(true, pred):
    return sum_squared_error(true,pred), mean_squared_error(true,pred), root_mean_squared_error(true,pred)
